...
export async function downloadUrlAs(
    ...
    // "downloadUrlAs(...)" is not supported by native messengers <= 0.1.9
    ...
    // We want to return a promise that will resolve once the file has
    ...
    return new Promise((resolve, reject) => {
        const onDownloadComplete = async downloadDelta => {
            ...
            // Note: this might be a little too drastic. For example,
            // files that encounter a problem while being downloaded and
            // the download of which is restarted by a user won't be
            // moved This seems acceptable for now as taking all states
            // into account seems quite difficult
            if (
                downloadDelta.state &&
