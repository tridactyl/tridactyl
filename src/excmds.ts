 ...
 *
 * Lastly, themes can be loaded from URLs with `:colourscheme --url [url] [themename]`. They are stored internally - if you want to update the theme run the whole command again. You can use `%` as a placeholder for the current URL.
 *
...
export async function colourscheme(...args: string[]) {
    const option = arg.lib({"--url": String}, {argv: args, allowNegativePositional: true})
    let url = option["--url"]
