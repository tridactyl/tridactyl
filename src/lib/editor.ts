...
function needs_text(fn: editor_function): editor_function {
    return (text: string, selectionStart: number, selectionEnd: number) => {
        if (text.length === 0 || selectionStart === null || selectionStart === undefined)
            return [null, null, null]
        return fn(text, selectionStart, (typeof selectionEnd) == "number" ? selectionEnd : selectionStart)
    }
}
 ...
 **/
export const delete_char = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
 ...
 **/
export const delete_backward_char = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    if (selectionStart != selectionEnd) {
        text = text.substring(0, selectionStart) + text.substring(selectionEnd)
    } else {
 ...
 **/
export const transpose_chars = wrap_input((text, selectionStart, selectionEnd) => {
    if (text.length < 2)
        return [null, null, null]
 ...
 */
function applyWord(text, selectionStart, selectionEnd, fn: (string) => string): [string, number, number] {
 ...
 **/
export const transpose_words = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    if (selectionStart >= text.length) {
        selectionStart = text.length - 1
    }
    ...
    return [beginning + firstWord + text.substring(secondBoundaries[1]), selectionStart, null]
}))
 ...
 **/
export const upcase_word = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
 ...
 **/
export const downcase_word = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
 ...
 **/
export const capitalize_word = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
 ...
 **/
export const kill_line = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
 ...
 **/
export const backward_kill_line = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    // If the caret is at the beginning of a line, join the lines
    if (selectionStart > 0 && text[selectionStart - 1] == "\n") {
        return [text.substring(0, selectionStart - 1) + text.substring(selectionStart), selectionStart, null]
    }
    let newLine
    ...
    return [text.substring(0, newLine) + text.substring(selectionStart), newLine, null]
}))
 ...
 **/
export const kill_whole_line = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    ...
    // Remove everything between the newline and the caret
    return [text.substring(0, firstNewLine) + text.substring(secondNewLine), firstNewLine, null]
}))
 ...
 **/
export const kill_word = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    let boundaries = getWordBoundaries(text, selectionStart, false)
    if (selectionStart > boundaries[0] && selectionStart < boundaries[1]) boundaries[0] = selectionStart
    // Remove everything between the newline and the caret
    return [text.substring(0, boundaries[0]) + text.substring(boundaries[1] + 1), boundaries[0], null]
}))
 ...
 **/
export const backward_kill_word = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    let boundaries = getWordBoundaries(text, selectionStart, true)
    if (selectionStart > boundaries[0] && selectionStart < boundaries[1]) boundaries[1] = selectionStart
    // Remove everything between the newline and the caret
    return [text.substring(0, boundaries[0]) + text.substring(boundaries[1]), boundaries[0], null]
}))
 ...
 **/
export const beginning_of_line = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    while (text[selectionStart - 1] != undefined && text[selectionStart - 1] != "\n") selectionStart -= 1
 ...
 **/
export const end_of_line = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    while (text[selectionStart] != undefined && text[selectionStart] != "\n") selectionStart += 1
    return [null, selectionStart, null]
}))
 ...
 **/
export const backward_char = wrap_input((text, selectionStart, selectionEnd) => {
    return [null, selectionStart - 1, null]
})
 ...
 **/
export const forward_word = wrap_input(needs_text((text, selectionStart, selectionEnd) => {
    let boundaries = getWordBoundaries(text, selectionStart, false)
    if (selectionStart >= boundaries[0] && selectionStart < boundaries[1]) boundaries = getWordBoundaries(text, boundaries[1], false)
 ...
 **/
export const backward_word = wrap_input((text, selectionStart, selectionEnd) => {
    if (selectionStart == 0) return [null, null, null]
    let boundaries = getWordBoundaries(text, selectionStart, true)
    if (selectionStart >= boundaries[0] && selectionStart < boundaries[1]) boundaries = getWordBoundaries(text, boundaries[0] - 1, true)
    return [null, boundaries[0], null]
})
